...
sed -i '' -e 's/const { MigrationInterface, QueryRunner } = require(\"typeorm\");/import typeorm from \"typeorm\";\n\nconst { MigrationInterface, QueryRunner } = typeorm;/g' migrations/*
sed -i '' -e 's/module.exports = class/export default class/g' migrations/*