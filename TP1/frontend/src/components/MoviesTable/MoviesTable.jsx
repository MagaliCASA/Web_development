...
function MoviesTable(props) {
  ...
  async function getPopularMovies(basepage) {
    try {
      ...
      if (props.search == "") {
        demande = "movie/popular";
      } else {
        demande = "search/movie";
        //const response = await axios.get(`${baseUrl}search/movie?api_key=${apiKey}&query=${props.search}&page=${props.page}`);
      }
      //&page=${props.page}
      const response = await axios.get(`${baseUrl}${demande}?api_key=${apiKey}&page=${basepage}&query=${props.search}`);
